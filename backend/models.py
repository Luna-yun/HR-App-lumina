...
class User(BaseModel):
    ...
    # Status fields
    is_verified: bool = True  # No email verification required
...
class Attendance(BaseModel):
    ...
    status: str = AttendanceStatus.NOT_CHECKED_IN
    notes: str = ""
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
...
class JobPosting(BaseModel):
    ...
    status: str = RecruitmentStatus.OPEN
    created_by: str
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
...
class Applicant(BaseModel):
    ...
    resume_url: str = ""
    cover_letter: str = ""
    status: str = ApplicantStatus.NEW
    notes: str = ""
...
class KnowledgeDocument(BaseModel):
    ...
    file_size: int
    content_hash: str  # To avoid duplicate processing
    chunk_count: int = 0
    uploaded_by: str
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class ChatMessage(BaseModel):
    ...
    content: str
    sources: List[str] = Field(default_factory=list)  # Document references
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class ChatResponse(BaseModel):
    ...
    sources: List[dict]
    session_id: str
    reasoning: str  # Explanation of how the answer was derived
